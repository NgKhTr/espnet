...
${cmd} JOB=1:${nj} ${logdir}/feat_to_shape.JOB.log \
    pyscripts/feats/feat-to-shape.py --verbose ${verbose} ${preprocess_opt} ${filetype_opt} \
    scp:${logdir}/feats.JOB.scp ${logdir}/shape.JOB.scp || { cat $(grep -l -i error "${logdir}"/feat_to_shape.*.log) ; exit 1; }
